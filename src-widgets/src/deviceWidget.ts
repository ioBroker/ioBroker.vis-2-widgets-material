...
const DEVICE_ICONS: Partial<Record<Types, string>> = {
    [Types.blind]:
    ...
    [Types.rgb]: rgbIcon,
    [Types.rgbSingle]: rgbIcon,
    [Types.rgbwSingle]: rgbIcon,
    [Types.hue]: rgbIcon,
    [Types.ct]: rgbIcon,
    [Types.socket]:
...
function simpleState(
    ...
    if (settings?.standardIcons && DEVICE_ICONS[device.deviceType as Types]) {
        settings.icon = DEVICE_ICONS[device.deviceType as Types];
    }
...
export function getDeviceWidget(device: DetectorDevice, standardIcons?: boolean): null | SingleWidget {
    ...
    if (device.deviceType === 'thermostat') {
        ...
        const modeState: ioBroker.StateCommon | undefined = mode?.common as ioBroker.StateCommon;
        if (modeState?.states) {
            if (Array.isArray(modeState.states)) {
                const states: Record<string, string> = {};
                modeState.states.forEach(key => (states[key] = key));
                modeState.states = states;
            }
            widget.data.count = Object.keys(modeState.states).length;
            Object.keys(modeState.states).forEach((_mode, i) => {
                widget.data[`g_modes-${i}`] = true;
                widget.data[`title${i}`] = (modeState.states as Record<string, string>)[_mode].toString().toUpperCase();
            });
        }
...
export const getDeviceWidgetOnePage = (
    device: DetectorDevice,
    widgetId: SingleWidgetId,
    parentWidget: SingleWidget,
    viewObj: View,
    standardIcons?: boolean,
): boolean => {
    const addSwitch = (
        ...
        settings: {
            ...
            iconSmall?: string;
            type?: WidgetType;
            infoInactiveIcon?: string;
            infoActiveIcon?: string;
        ...
        if (standardIcons) {
            ...
            } else if (DEVICE_ICONS[device.deviceType as Types]) {
                settings.icon = DEVICE_ICONS[device.deviceType as Types];
            }
        }
        ...
        if (_set) {
            ...
            if (role === 'value.temperature' && settings.type === Types.info) {
                parentWidget.data[`title${parentWidget.data.count}`] = Generic.t('Temperature');
            } else if (role === 'value.humidity' && settings.type === Types.info) {
            ...
            if (settings) {
                Object.keys(settings)
                    .filter(s => (settings as Record<string, any>)[s] !== undefined)
                    .forEach(
                        setting =>
                            (parentWidget.data[`${setting}${parentWidget.data.count}`] = (
    ...
    if (device.deviceType === Types.rgb) {
        ...
        const power =
            device.states.find(
                state => state.name === 'ON' && (state.common as ioBroker.StateCommon).write !== false,
            ) || undefined;
    ...
    if (device.deviceType === 'rgbSingle') {
        const rgb = device.states.find(state => state.name === 'RGB')!;
        const power =
            device.states.find(
                state => state.name === 'ON' && (state.common as ioBroker.StateCommon).write !== false,
            ) || undefined;
    ...
    if (device.deviceType === 'rgbwSingle') {
        const rgbw = device.states.find(state => state.name === 'RGBW')!;
        const power =
            device.states.find(
                state => state.name === 'ON' && (state.common as ioBroker.StateCommon).write !== false,
            ) || undefined;
    ...
    if (device.deviceType === 'hue') {
        const hue = device.states.find(state => state.name === 'HUE')!;
        const power =
            device.states.find(
                state => state.name === 'ON' && (state.common as ioBroker.StateCommon).write !== false,
            ) || undefined;
    ...
    if (device.deviceType === 'ct') {
        const temperature = device.states.find(state => state.name === 'TEMPERATURE')!;
        const power =
            device.states.find(
                state => state.name === 'ON' && (state.common as ioBroker.StateCommon).write !== false,
            ) || undefined;
